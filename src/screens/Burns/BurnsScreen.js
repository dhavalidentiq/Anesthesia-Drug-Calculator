...
export default BurnsScreen = ({ navigation, route }) => {
  ...
  const npoColor = '#0051C6';
  const sflColor = '#E5431E';
  ...
  const _pediatricSelectBodyParts = bodyPart => {
    ...
    if (pediatricBodyParts.some(item => item?.bodypart === bodyPart)) {
      // If selected, find the item and remove it from the list
      const item = pediatricBodyParts.find(item => item?.bodypart === bodyPart);
        ...
        <View style={styles.disclaimerMainView}>
          {(!peds ? totalSumBurns : _pediatricTotalSumBurns) <= 20 && (
            <Text
              ...
              numberOfLines={1}
              adjustsFontSizeToFit>
              *The Parkland Formula is for greater than or equal to 20% of total
              body area burns
            </Text>
          )}
        </View>
...
// const BurnsScreen: React.FC = ({ navigation, route }: any) => {
//   const { peds, age, weight, bmi } = route.params;
//   const [isFrontSide, setIsFrontSide] = useState<boolean>(true);
//   const [isObesePatient, setIsObesePatient] = useState<boolean>(false);
//   const [modalOpen, setModalOpen] = useState(false);
//   const [hoursAgo, setHoursAgo] = useState<number>(0);
//   const [frontBodyParts, setFrontBodyParts] = useState<BodyPart[]>([]);
//   const [pediatricBodyParts, setPediatricBodyParts] = useState<BodyPart[]>([]);
//   const [pediatricBurns, setPediatricBurns] = useState<any>({});
//   const { top, bottom } = useSafeAreaInsets();
//   const isDarkMode = useSelector(
//     (state: any) => state.darkModeReducer.isDarkMode
//   );
//   const theme = isDarkMode ? darkTheme : lightTheme;
//   const backgroundColor = theme?.backgroundColor;
//   const textColor = theme?.textColor;
//   const segmentTabBackgroundColor = theme?.smokeBackgroundColor;
//   const npoColor = "blue";
//   const sflColor = "red";

//   const toggleSwitch = (context: string) => {
//     if (context === "Anterior") {
//       setIsFrontSide(true);
//     } else {
//       setIsFrontSide(false);
...
//   const _selectFrontBodyParts = (bodyPart: string) => {
//     // Check if the bodyPart is already selected in frontBodyParts
//     if (frontBodyParts.some((item) => item?.bodypart === bodyPart)) {
//       // If selected, find the item and remove it from the list
//       const item: any = frontBodyParts.find(
//         (item) => item?.bodypart === bodyPart
//       );
//       const removeBodyPart: any = frontBodyParts.filter(
//         (i) => i?.bodypart !== bodyPart
//       );
//       setFrontBodyParts([...removeBodyPart, item]);
//     } else {
...
//   const _adultOnValueChange = (val: any) => {
//     const array = [...frontBodyParts];
//     let totalBurn: any =
//       (array[frontBodyParts?.length - 1]?.initialValue * val) / 100;
//     array[frontBodyParts?.length - 1] = {
...
//   let totalSumBurns: any = frontBodyParts.reduce(function (prev, current) {
//     return prev + parseFloat(current?.totalBurn);
//   }, 0);

//   const getDataOfBurnByAge = async (age: any) => {
//     let temp = {
//       head: 18,
//       legs: 28,
//     };
...
//   const _pediatricSelectBodyParts = (bodyPart: string) => {
//     // Check if the bodyPart is already selected in pediatricBodyParts
//     if (pediatricBodyParts.some((item) => item?.bodypart === bodyPart)) {
//       // If selected, find the item and remove it from the list
//       const item: any = pediatricBodyParts.find(
//         (item) => item?.bodypart === bodyPart
//       );
//       const removeBodyPart: any = pediatricBodyParts.filter(
//         (i) => i?.bodypart !== bodyPart
//       );
//       setPediatricBodyParts([...removeBodyPart, item]);
//     } else {
...
//   const _pediatricOnValueChange = (val: any) => {
//     const array = [...pediatricBodyParts];
//     let totalBurn: any =
//       (array[pediatricBodyParts?.length - 1]?.initialValue * val) / 100;
//     array[pediatricBodyParts?.length - 1] = {
...
//   let _pediatricTotalSumBurns: any = pediatricBodyParts.reduce(function (
//     prev,
//     current
//   ) {
//     return prev + parseFloat(current?.totalBurn);
...
//         isFrontSide={isFrontSide}
//         isObesePatient={isObesePatient}
//         hoursvalue={(val: any) => setHoursAgo(val)}
//         _adultOnValueChange={(val: any) => _adultOnValueChange(val)}
//         _pediatricOnValueChange={(val: any) => _pediatricOnValueChange(val)}
//         _selectFrontBodyParts={(val: any) => _selectFrontBodyParts(val)}
//         _pediatricSelectBodyParts={(val: any) => _pediatricSelectBodyParts(val)}
//       />
...
//           color="grey"
//         />
//         <Text style={{ color: textColor }}> Rationals</Text>
//       </TouchableOpacity>
